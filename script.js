...
function createSunflower(x, y, forceToField = false) {
    ...
    if (y < fieldTop || forceToField) {
        y = fieldTop + (container.offsetHeight / 2) * 0.8 * Math.random(); // parte inferior, pero no pegado abajo
    }

    // Posicionar el girasol usando left/top relativos al contenedor
    sunflower.style.left = `${x - 30}px`; // 30 = mitad del ancho del girasol
    sunflower.style.top = `${y - 60}px`;  // 60 = altura para centrar la flor
    ...
    const swayDuration = 3 + Math.random() * 2; // entre 3 y 5 segundos
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => createInitialSunflowers(8), 200);
